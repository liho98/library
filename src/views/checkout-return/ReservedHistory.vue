      ...
      .then(querySnapshot => {
        querySnapshot.forEach(doc => {
          const data = {
            ...
            reserve_at: doc.data().reserve_at.toDate(),
            reserve_at_str: doc
              .data()
              .reserve_at.toDate()
            ...
            due_date_str: doc
              ...
              .due_date.toDate()
              .toLocaleString("en-GB", { timeZone: "UTC", hour12: true }),
            status: doc.data().status.replace(/\b\w/g, l => l.toUpperCase()),
        ...
        Object.keys(this.reserve).forEach(key => {
          db.collection("books")
            ...
            .get()
            .then(snapshot => {
              this.reserve[key].book_title = snapshot.data().title;
            });
        });

        Object.keys(this.reserve).forEach(key => {
          if (
            this.reserve[key].status === "collected" ||
            (this.reserve[key].checkout_did != undefined &&
              this.reserve[key].checkout_did != "")
          ) {
            db.collection("checkout")
              .doc(this.reserve[key].checkout_did)
              .get()
              .then(doc => {
                this.reserve[
                  key
                ].borrowed_date = doc.data().borrowed_date.toDate();
                this.reserve[key].borrowed_date_str = doc
                  .data()
